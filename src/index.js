...
program
    ...
    .option(
        '-f, --format <string>',
        'Set format for each page. For example, \'A4\' or \'A3\'',
    )
    .addHelpCommand()
    .parse(process.argv);
...
program.opts().format.toUpperCase() :
null;
...
console.log(
    format ?
    `Use '${format}', for each page` :
    'Use default resolution of each image for each page',
);
